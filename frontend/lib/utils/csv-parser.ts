...
import { getPM10Category, getPM25Category, shouldStoreOnBlockchain } from './air-quality';
 ...
 */
export function parseCSV(csvData: string, location: string = 'Default'): Promise<ProcessedReading[]> {
          ...
          results.data.forEach((row: any, index: number) => {
            ...
            try {
              ...
              if (timeParts.length === 2) {
                ...
                timestamp.setHours(0, minutes, Math.floor(seconds), (seconds % 1) * 1000);
              } else {
                // Fallback to current time
                timestamp = new Date();
              }
            } catch (e) {
              console.warn(`Failed to parse timestamp "${timeString}", using current time`);
              timestamp = new Date();
            }
 ...
 * @returns Promise with array of processed readings
 */
export async function processCSVFile(file: File, location?: string): Promise<ProcessedReading[]> {
  return new Promise((resolve, reject) => {
